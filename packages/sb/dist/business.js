...
export class BusinessService {
     ...
     */
    static async createBusinessReport(userId, reportType, title, dateRangeStart, dateRangeEnd, filters) {
        try {
     ...
     */
    static async getBusinessReports(userId, limit = 20, offset = 0) {
        try {
            const { data, error } = await supabase
     ...
     */
    static async getBusinessMetrics(userId, startDate, endDate) {
        try {
     ...
     * Get inventory items
     */
    static async getInventoryItems(userId, limit = 50, offset = 0) {
        try {
            const { data, error } = await supabase
     ...
     */
    static async createInventoryItem(userId, itemData) {
        try {
            const { data, error } = await supabase
                .from('inventory_items')
                .insert({
     ...
     */
    static async updateInventoryQuantity(inventoryItemId, quantityChange, movementType, reason, referenceId, referenceType, createdBy) {
        try {
     ...
     */
    static async getInventoryMovements(inventoryItemId, limit = 50, offset = 0) {
        try {
            const { data, error } = await supabase
     ...
     * Get business goals
     */
    static async getBusinessGoals(userId, status) {
        try {
            let query = supabase
                .from('business_goals')
                .select('*')
     ...
     */
    static async createBusinessGoal(userId, goalData) {
        try {
            const { data, error } = await supabase
                .from('business_goals')
                .insert({
     ...
     */
    static async getBusinessNotifications(userId, unreadOnly = false, limit = 20, offset = 0) {
        try {
            let query = supabase
     ...
     */
    static async getLowInventoryAlerts(userId) {
        try {
            const { data, error } = await supabase
                .from('inventory_items')
                .select('*')
     ...
     */
    static async getBusinessDashboard(userId) {
        try {
            const [metrics, goals, notifications, lowInventory] = await Promise.all([
                this.getBusinessMetrics(userId, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], new Date().toISOString().split('T')[0]),
