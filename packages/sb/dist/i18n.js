...
export class I18nService {
     ...
     */
    static async getTranslations(languageCode, namespace = 'common') {
     ...
     */
    static async getTranslation(key, languageCode = 'en', namespace = 'common') {
        try {
     ...
     */
    static async getLocalizedContent(contentType, contentId, languageCode = 'en', fieldName) {
        try {
     ...
     */
    static async setLocalizedContent(contentType, contentId, languageCode, fieldName, fieldValue) {
        try {
     ...
     */
    static formatDate(date, languageCode = 'en', timezone = 'UTC') {
     ...
     */
    static formatTime(date, languageCode = 'en', timezone = 'UTC', format = '12h') {
        const dateObj = typeof date === 'string' ? new Date(date) : date;
        try {
            return new Intl.DateTimeFormat(languageCode, {
                timeZone: timezone,
     ...
     */
    static formatCurrency(amount, currencyCode = 'RUB', languageCode = 'en') {
        try {
     ...
     */
    static formatNumber(number, languageCode = 'en', options) {
        try {
     ...
     */
    static formatRelativeTime(date, languageCode = 'en', timezone = 'UTC') {
        const dateObj = typeof date === 'string' ? new Date(date) : date;
        const now = new Date();
...
I18nService.translationsCache = new Map();
I18nService.languagesCache = [];
// Utility functions for common i18n operations
export const t = (key, languageCode = 'en', namespace = 'common') => {
    return I18nService.getTranslation(key, languageCode, namespace);
};
export const formatPrice = (amount, currencyCode = 'RUB', languageCode = 'en') => {
    return I18nService.formatCurrency(amount, currencyCode, languageCode);
};
export const formatDate = (date, languageCode = 'en', timezone = 'UTC') => {
    return I18nService.formatDate(date, languageCode, timezone);
};
export const formatTime = (date, languageCode = 'en', timezone = 'UTC', format = '12h') => {
    return I18nService.formatTime(date, languageCode, timezone, format);
};
export const formatRelativeTime = (date, languageCode = 'en', timezone = 'UTC') => {
