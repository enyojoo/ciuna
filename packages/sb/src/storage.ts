...
export const storage = {
  ...
  // Delete bucket
  async deleteBucket(bucketName: string): Promise<{ data: any; error: any }> {
    const { data, error } = await supabase.storage
...
export const storageHelpers = {
  ...
  async uploadListingPhoto(listingId: number, file: File, index: number): Promise<string> {
  ...
  async uploadProductPhoto(productId: number, file: File, index: number): Promise<string> {
  ...
  async uploadServicePhoto(serviceId: number, file: File, index: number): Promise<string> {
  ...
  // Upload vendor logo
  async uploadVendorLogo(vendorId: number, file: File): Promise<string> {
  ...
  // Upload document
  async uploadDocument(userId: string, documentType: string, file: File): Promise<string> {
