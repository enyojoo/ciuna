...
## 5. Step B — Ciuna strict 3% USDT bridge (margin)

```text
ciuna_buy_raw(X)  = B'(X) × 1.03
ciuna_sell_raw(X) = S'(X) × 0.97
```
...
Aggregates + ±3% can yield **`ciuna_buy_raw < ciuna_sell_raw`**, which is awkward for a broker-like quote. Enforce a minimum relative wedge **`m`** (tier table §8).
