...
export default function ProviderDashboardPage() {
  ...
  const loadProviderData = async () => {
    try {
      ...
      const providerData = await db.serviceProviders.getByProfileId(user?.id || '');
      setProvider(providerData);

      // Get services
      const servicesData = await db.services.getAll({ provider_id: providerData.id }, 1, 10);
      setServices(servicesData.data);

      // Get bookings
      const bookingsData = await db.bookings.getAll({ provider_id: providerData.id }, 1, 20);
      setBookings(bookingsData.data);

      // Calculate stats
      const activeServices = servicesData.data.filter(s => s.status === 'ACTIVE').length;
      const pendingBookings = bookingsData.data.filter(b => b.status === 'PENDING').length;
      const completedBookings = bookingsData.data.filter(b => b.status === 'COMPLETED').length;
      ...
      setStats({
        ...
        pendingBookings,
        completedBookings,
        totalRevenue: providerData.total_revenue || 0,
        monthlyRevenue: providerData.monthly_revenue || 0,
        averageRating: providerData.rating || 0,
        totalReviews: providerData.review_count || 0,
      });
    } catch (error) {
