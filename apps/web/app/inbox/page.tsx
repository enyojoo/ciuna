...
export default function InboxPage() {
  ...
  const loadConversations = async () => {
    try {
      setLoading(true);
      const { data } = await db.conversations.getAll({ user_id: user?.id }, 1, 50);
  ...
  const loadMessages = async (conversationId: number) => {
    try {
      const { data } = await db.messages.getAll({ conversation_id: conversationId }, 1, 100);
      setMessages(data);
    } catch (error) {
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <Avatar
                            ...
                            fallback={otherParticipant?.user.email.charAt(0) || 'U'}
                            size="md"
                          />
            ...
            <Card className="h-full flex flex-col">
              ...
              <div className="p-4 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <Avatar
                      src={getOtherParticipant(selectedConversation)?.user.avatar_url || undefined}
                      fallback={getOtherParticipant(selectedConversation)?.user.email.charAt(0) || 'U'}
                      size="md"
                    />
                    <div>
