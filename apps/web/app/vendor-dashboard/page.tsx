...
import type { VendorWithOwner, VendorProductWithRelations, OrderWithRelations } from '@ciuna/types';
  ...
  const loadVendorData = async () => {
    try {
      ...
      setStats({
        ...
        monthlyRevenue: vendorData.monthly_revenue || 0,
      });
    } catch (error) {
                ...
                {recentOrders.map((order) => (
                  <div key={order.id} className="flex items-center space-x-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500">
                        {formatPrice(order.total_amount_rub)} • {order.buyer.email}
                      </p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Badge variant={
