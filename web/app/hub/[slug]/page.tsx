...
export default function HubServiceCatalogPage() {
  ...
  useLayoutEffect(() => {
    ...
    const staleLines = readStaleHubServiceLinesCache(cacheUserId)
    if (staleLines !== null) {
      setLines(staleLines)
      setLinesLoaded(true)
    }
  ...
  useEffect(() => {
    if (!user || !cacheUserId || !isMarketplaceLine) return
    let cancelled = false
    setLoadingVendors(true)
    ;(async () => {
      try {
        const res = await fetchWithAuth(`/api/hub/vendors?service_line=${encodeURIComponent(slug)}`, { cache: "no-store" })
        if (!res.ok) throw new Error("vendors")
        const data = await res.json()
        if (!cancelled) setVendors((data.vendors || []) as HubVendorRow[])
      } catch {
  ...
  }, [user, cacheUserId, slug, isMarketplaceLine])
  ...
  const onCategoryFilterChange = (value: string) => {
    if (value === ALL_CATEGORIES_VALUE) {
      router.replace(`/hub/${slug}`)
      return
    }
    router.replace(`/hub/${slug}?category=${encodeURIComponent(value)}`)
  }
      ...
      {loading && products.length === 0 ? (
        ...
        <>
          <div className="flex min-w-0 items-center gap-3 pb-4">
            <h3 className="shrink-0 text-lg font-semibold text-gray-900">{t("hub.productsSectionTitle")}</h3>
            <div className="min-w-[1rem] flex-1 border-t border-dashed border-gray-300" aria-hidden />
